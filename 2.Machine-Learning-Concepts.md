...
**Note:**  [Deep Dive Into Binary Classification](https://learn.microsoft.com/en-us/training/modules/fundamentals-machine-learning/5-binary-classification)
...
**Note:** [Deep Dive into MultiClass Classification](https://learn.microsoft.com/en-us/training/modules/fundamentals-machine-learning/6-multiclass-classification)
...
### Clustering in Nonsupervised ML:
...
| Feature           | Description                               |
| ----------------- | ----------------------------------------- |
| **Input**         | Unlabeled data points (features only)     |
| **Output**        | Groups or clusters of similar data points |
| **Goal**          | Discover hidden structures or groupings   |
| **Training Data** | No labels, only input features            |

Clustering or also known as cluster analysis is a type of machine learning technique, which aims to group the unlabelled dataset. In a precise way, we can define Clustering as a technique of grouping the data points into different classes of clusters, consisting of data points having similar attributes. The objects with distinguishing similarities remain in a group that has less or almost no similarities with another group.

The algorithm can achieve this by recognizing some similar patterns in the unlabelled dataset like shape, size, color, behavior, and many such attributes, and divides them as per the presence and absence of those similar patterns present in the dataset.

Clustering is a type of unsupervised learning method; hence we do not any supervision for the algorithm, and it deals with the unlabeled dataset in an efficient way.

After deploying this clustering technique, we provide each cluster or group with a cluster ID. Machine Learning systems can make use of this id to simplify the processing of large and complex datasets in an effective way.

**Example:**

<p align="center">
  <img src="images/ML-13.png" alt="Description of my awesome image" width="600">
</p>

**Real-Life Examples:**

| Problem               | Inputs (Features)              | Output (Clusters)                        |
| --------------------- | ------------------------------ | ---------------------------------------- |
| Customer segmentation | Purchase history, demographics | Groups of similar customers              |
| Document clustering   | Word frequencies in documents  | Topics or categories of documents        |
| Image segmentation    | Pixel data                     | Different regions or objects in an image |

**Common Clustering Algorithms:**

| Algorithm                         | Description                                                                 |
| --------------------------------- | --------------------------------------------------------------------------- |
| **K-Means**                       | Partitions data into *k* clusters by minimizing distance to cluster centers |
| **Hierarchical**                  | Builds a tree of clusters (dendrogram) from bottom-up or top-down           |
| **DBSCAN**                        | Finds clusters based on density (good for arbitrary shapes)                 |
| **Gaussian Mixture Models (GMM)** | Models data as a mixture of Gaussian distributions                          |


**Note:** [Deep Dive into Clustering](https://learn.microsoft.com/en-us/training/modules/fundamentals-machine-learning/7-clustering)

### Deep Learning:

Deep Learning is a specialized subset of machine learning that uses artificial neural networks with many layers (hence “deep”) to model complex patterns in data.
...
<p align="center">
  <img src="images/ML-14.png" alt="Description of my awesome image" width="600">
</p>
...
<p align="center">
  <img src="images/ML-15.png" alt="Description of my awesome image" width="600">
</p>
...
**Why Is Deep Learning Popular than machine learning models ?**

No Feature Extraction:

<p align="center">
  <img src="images/ML-16.png" alt="Description of my awesome image" width="600">
</p>

**Note:** [Deep Dive into Deep Learning](https://learn.microsoft.com/en-us/training/modules/fundamentals-machine-learning/8-deep-learning)

### Transformers:

The transformer architecture in machine learning is a powerful neural network design primarily used to process sequential data, such as language, by leveraging mechanisms called attention to model relationships within sequences.

**Core Components**:

* **Encoder-Decoder Structure:** The architecture consists of stacks of encoders and decoders.
  * The encoder reads the input and generates a set of representations.
  * The decoder uses these representations, along with its own input, to generate outputs such as translated text or generated sequences.

* **Self-Attention Mechanism:** This mechanism enables the model to weigh the importance of each token in the input sequence relative to the others. It helps capture long-range dependencies without needing recurrence.
* **Feed-Forward Neural Networks:** Both the encoder and decoder contain position-wise feed-forward layers, applied independently to each token’s representation, to further process the outputs of attention layers.
* **Positional Encoding:** Since transformers do not have built-in recurrence or convolution, positional encodings are added to the input embeddings to preserve the order of tokens in a sequence.

**Transformer Block Workflow**:

* Input text is first tokenized and transformed into numerical vectors (embeddings).
* Embeddings are processed in parallel (not step by step, as with RNNs).
* Each encoder layer performs self-attention and feed-forward operations, with residual connections and normalization for stability.
* Decoder layers include masked self-attention (to prevent attending to future tokens), cross-attention (attending to encoder outputs), and their own feed-forward networks.

**Why Transformers Are Powerful**:

* **Parallelism:** Unlike RNNs/LSTMs, Transformers process the entire sequence at once, allowing faster training.
* **Long-Range Dependencies:** Self-attention can relate any two positions in a sequence directly, making it better at understanding long texts.
* **Scalability**: Easily scaled up to billions of parameters (like GPT-4).

<p align="center">
  <img src="images/ML-20.PNG" alt="Description of my awesome image" width="600">
</p>
